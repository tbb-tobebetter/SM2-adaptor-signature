...
void SM2_AS_offline_PP_new(SM2_AS_offline_PP &pp)
{ 
...
/* free memory of PP */ 
void SM2_AS_offline_PP_free(SM2_AS_offline_PP &pp)
{ 
    EC_POINT_free(pp.g);
}


void SM2_AS_offline_KP_new(SM2_AS_offline_KP &keypair)
{
    keypair.pk = EC_POINT_new(group); 
    keypair.sk = BN_new(); 
}

void SM2_AS_offline_KP_free(SM2_AS_offline_KP &keypair)
{
...
void SM2_AS_offline_SIG_new(SM2_AS_offline_SIG &SIG)
{
    SIG.r = BN_new(); 
    SIG.z = BN_new();
...
void SM2_AS_offline_SIG_free(SM2_AS_offline_SIG &SIG)
{
    BN_free(SIG.r); 
    BN_free(SIG.z);
    EC_POINT_free(SIG.Z);
...
void SM2_AS_offline_PP_print(SM2_AS_offline_PP &pp)
{
    ECP_print(pp.g, "pp.g"); 
} 

void SM2_AS_offline_KP_print(SM2_AS_offline_KP &keypair)
{
    ECP_print(keypair.pk, "pk"); 
    BN_print(keypair.sk, "sk"); 
} 

void SM2_AS_offline_SIG_print(SM2_AS_offline_SIG &SIG)
{
...
void SM2_AS_offline_SIG_serialize(SM2_AS_offline_SIG &SIG, ofstream &fout)
{
    BN_serialize(SIG.r, fout); 
    BN_serialize(SIG.z, fout); 
} 

void SM2_AS_offline_SIG_deserialize(SM2_AS_offline_SIG &SIG, ifstream &fin)
{
...
/* Setup algorithm */ 
void SM2_AS_offline_Setup(SM2_AS_offline_PP &pp)
{ 
    EC_POINT_copy(pp.g, generator); 

//    #ifdef DEBUG
//    cout << "generate the public parameters for SM2_AS_offline Signature >>>" << endl; 
//    SM2_AS_offline_PP_print(pp); 
//    #endif
}

/* KeyGen algorithm */ 
void SM2_AS_offline_KeyGen(SM2_AS_offline_PP &pp, SM2_AS_offline_KP &keypair)
{ 
...
//    SM2_AS_offline_KP_print(keypair); 
//    #endif
}

void IY_Gen(SM2_AS_offline_PP &pp,EC_POINT *&pk, BIGNUM *&sk, IY &statwit, SM2_AS_offline_SIG &SIG)
{
    ...
    EC_POINT_mul(group, SIG.Z, NULL, statwit.Y, sk_1, bn_ctx); // SIG.Z=(sk+1)Y=(x+1)Y
...
void SM2_AS_offline_Sign(SM2_AS_offline_PP &pp, BIGNUM *&sk, EC_POINT *&Y, string &message, SM2_AS_offline_SIG &SIG)
{
    SM2_AS_offline_SIG sig; // define the signature
    BIGNUM *k = BN_new();
    BN_random(k);  
...
//    #ifdef DEBUG
//        cout << "SM2_AS_offline signature generation finishes >>>" << endl;
//        SM2_AS_offline_SIG_print(SIG);  
//    #endif
...
bool SM2_AS_offline_Verify(SM2_AS_offline_PP &pp, EC_POINT *&pk, EC_POINT *&Y, string &message, SM2_AS_offline_SIG &SIG)
{
    bool Validity;       
