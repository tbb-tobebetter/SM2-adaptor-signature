...
void test_SM2_AS()
{
    ...
    SM2_AS_KP keypair[TEST_NUM];
    for(auto i = 0; i < TEST_NUM; i++)
    {
    SM2_AS_KP_new(keypair[i]);
    }
    ...
    SM2_AS_SIG SIG[TEST_NUM];
    for(auto i = 0; i < TEST_NUM; i++)
    { 
    SM2_AS_SIG_new(SIG[i]); 
    }
    ...
    for(auto i = 0; i < TEST_NUM; i++)
    { 
    SM2_AS_Sign(pp, keypair[i].pk,keypair[i].sk, statwit[i].Y, message, SIG[i]);
    }
    end_time = chrono::steady_clock::now(); 
    running_time = end_time - start_time;
    ...
    for(auto i = 0; i < TEST_NUM; i++)
    { 
    SM2_AS_Verify(pp, keypair[i].pk, statwit[i].Y, message, SIG[i]);
    }
    end_time = chrono::steady_clock::now(); 
    running_time = end_time - start_time;
...
{  
    ...
    SplitLine_print('-'); 
    cout << "SM2_AS Signature test begins >>>>>>" << endl; 
    SplitLine_print('-'); 

    test_SM2_AS();

    SplitLine_print('-'); 
    cout << "SM2_AS Signature test finishes <<<<<<" << endl; 
    SplitLine_print('-'); 
